...
async def print_nicer_llm_response(*, message: types.Content, context: AgentContext):
    for part in message.parts:
        if not part.text:
            continue
        print()
...
async def print_nicer_tool_result(
    *,
    result: ToolResult,
    call_name: str,
    call_args: dict[str, Any],
...
async def main() -> None:
    ...
    agent = Agent()
    agent.on("on_tool_result", print_nicer_tool_result)
    agent.on("on_model_response", print_nicer_llm_response)
