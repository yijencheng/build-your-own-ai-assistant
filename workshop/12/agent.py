...
import re
from typing import Any, Protocol, Awaitable, Literal, TypeAlias, overload
from session import (
    FCMetadata,
...
You have a memory folder which stores timestamped memories of past conversations for each day at the file path memory/dd-mm-yyyy. If the user references something from a previous conversation, check there first. If it doesn't exist, assume that there is no data from that time/period.

Be concise, helpful and if you're unsure about something just ask for clarification rather than guesss.
"""
...
async def print_tool_result(
    *,
    ...
    edit_names = {"edit"}
    if call_name.lower() in edit_names:
        old_str = str(call_args.get("old_str", ""))
        new_str = str(call_args.get("new_str", ""))
        path = str(call_args.get("path", "<unknown>"))
        print(f"[bold]Edit[/bold] {path}")
    ...
    read_names = {"readfile"}
    if call_name.lower() in read_names:
        path = str(call_args.get("path", "<unknown>"))
        status = "[green]✓[/green]" if not result.error else "[red]✗[/red]"
        print(f"{status} [bold]ReadFile[/bold] {path}")
        payload = result.response.get("file_content")
        if isinstance(payload, str):
            content = extract_tag_block(payload, "content").strip("\n")
    ...
    bash_names = {"bash"}
    if call_name.lower() in bash_names:
        command = str(call_args.get("command", ""))
        working_dir = str(call_args.get("working_dir", "."))
        status = "[green]✓[/green]" if not result.error else "[red]✗[/red]"
        print(f"{status} [bold]Bash[/bold] `{command}` (cwd: {working_dir})")

        payload = result.response.get("result")
        if isinstance(payload, str):
            output = extract_tag_block(payload, "output").strip("\n")
            print(
                Panel(
                    Syntax(output, "console", line_numbers=True, word_wrap=False),
...
async def main() -> None:
    ...
    agent = Agent(session_manager=SessionManager(base_dir=base_dir))
