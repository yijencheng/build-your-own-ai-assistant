...
import importlib
from agent_tools import AgentTool, ToolResult, AgentContext
...
class AgentRuntime:
    ...
    def maybe_reload_runtime(self) -> None:
        ...
        reloaded_tools = ", ".join(sorted(self.tools))
        print(f"[cyan]Reloaded tools:[/cyan] {reloaded_tools or '(none)'}")
        self._last_modified = self._tools_file.stat().st_mtime
...
async def run(
    ...
    completion = await client.aio.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            system_instruction=f"You are a minimal coding agent called Koroku. You can modify your tools based off the file in {runtime._tools_file}",
        ),
    )
